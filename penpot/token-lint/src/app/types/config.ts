...
export interface Config {
  limits: {
    ...
    minWidth: number
    minHeight: number
    /** Shapes processed per event-loop tick during an audit (streaming/interruptibility). */
    auditChunkSize: number
    /** Throttle, in ms, between SHAPES_SCANNED progress messages. */
    auditProgressIntervalMs: number
  }
  env: {
    isDev: boolean
