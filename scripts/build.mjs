...
// its outDir to <repo-root>/dist/<platform>/<name>/, so this script only
// needs to trigger the right build script per matched workspace.
//
...
if (plugins.length === 0) {
  console.error(`No plugin workspace found${target ? ` for '${target}'` : ''}`)
  process.exit(1)
}
